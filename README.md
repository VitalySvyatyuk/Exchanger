...
- **A small connection pool**: the app under test uses 5 database connections, like Prisma's default on a 2-core CI runner. This exposed a bug that a large local pool hid: a purchase looked up the lot on a second connection while holding its transaction's connection, so simultaneous buyers could starve the pool until their transactions timed out.
